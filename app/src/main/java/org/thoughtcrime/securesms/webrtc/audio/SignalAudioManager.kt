...
sealed class SignalAudioManager(protected val context: Context, protected val eventListener: EventListener?) {
  ...
  protected var selectedAudioDevice: AudioDevice = AudioDevice(AudioDeviceType.NONE)
  ...
  class AudioDevice(var audioDeviceType: AudioDeviceType) : Serializable {
    var Id : ParcelUuid? = null
  }
...
class FullSignalAudioManager(context: Context, eventListener: EventListener?) : SignalAudioManager(context, eventListener), AudioDeviceUpdatedListener {
  ...
  private var userSelectedAudioDevice: AudioDevice = AudioDevice(AudioDeviceType.NONE)
  ...
  override fun onAudioDeviceUpdated() {
    ...
    val newAudioDevices = mutableSetOf(AudioDevice(AudioDeviceType.SPEAKER_PHONE))

    if (signalBluetoothManager.state.hasDevice()) {
      newAudioDevices += AudioDevice(AudioDeviceType.BLUETOOTH)
    }

    if (hasWiredHeadset) {
      newAudioDevices += AudioDevice(AudioDeviceType.WIRED_HEADSET)
    } else {
      autoSwitchToWiredHeadset = true
      if (androidAudioManager.hasEarpiece(context)) {
        newAudioDevices += AudioDevice(AudioDeviceType.EARPIECE)
      }
    }
    ...
    if (signalBluetoothManager.state == SignalBluetoothManager.State.UNAVAILABLE && userSelectedAudioDevice.audioDeviceType == AudioDeviceType.BLUETOOTH) {
      userSelectedAudioDevice = AudioDevice(AudioDeviceType.NONE)
    }

    if (hasWiredHeadset && autoSwitchToWiredHeadset) {
      userSelectedAudioDevice.audioDeviceType = AudioDeviceType.WIRED_HEADSET
      autoSwitchToWiredHeadset = false
    }

    if (!hasWiredHeadset && userSelectedAudioDevice.audioDeviceType == AudioDeviceType.WIRED_HEADSET) {
      userSelectedAudioDevice.audioDeviceType = AudioDeviceType.NONE
    }

    val needBluetoothAudioStart = signalBluetoothManager.state == SignalBluetoothManager.State.AVAILABLE &&
      (userSelectedAudioDevice.audioDeviceType == AudioDeviceType.NONE || userSelectedAudioDevice.audioDeviceType == AudioDeviceType.BLUETOOTH || autoSwitchToBluetooth) && !androidAudioManager.isBluetoothScoOn
    ...
    val needBluetoothAudioStop = (signalBluetoothManager.state == SignalBluetoothManager.State.CONNECTED || signalBluetoothManager.state == SignalBluetoothManager.State.CONNECTING) &&
      (userSelectedAudioDevice.audioDeviceType != AudioDeviceType.NONE && userSelectedAudioDevice.audioDeviceType != AudioDeviceType.BLUETOOTH)
    ...
    if (needBluetoothAudioStart && !needBluetoothAudioStop) {
      if (!signalBluetoothManager.startScoAudio()) {
        //audioDevices.remove(AudioDeviceType.BLUETOOTH)
        audioDeviceSetUpdated = true
      }
    }

    if (autoSwitchToBluetooth && signalBluetoothManager.state == SignalBluetoothManager.State.CONNECTED) {
      userSelectedAudioDevice.audioDeviceType = AudioDeviceType.BLUETOOTH
    ...
    val newAudioDevice: AudioDeviceType = when {
      audioDevices.contains(userSelectedAudioDevice) -> userSelectedAudioDevice.audioDeviceType
      audioDevices.contains(defaultAudioDevice) -> defaultAudioDevice.audioDeviceType
      else -> AudioDeviceType.SPEAKER_PHONE
    }

    if (newAudioDevice != selectedAudioDevice.audioDeviceType || audioDeviceSetUpdated) {
      setAudioDevice(AudioDevice(newAudioDevice))
  ...
  override fun setDefaultAudioDevice(recipientId: RecipientId?, newDefaultDevice: AudioDevice, clearUserEarpieceSelection: Boolean) {
    ...
    if (clearUserEarpieceSelection && userSelectedAudioDevice.audioDeviceType == AudioDeviceType.EARPIECE) {
  ...
  override fun selectAudioDevice(recipientId: RecipientId?, device: Int, isId: Boolean) {
    ...
    val mappedDevice = AudioDeviceType.values()[device]
    val actualDevice: AudioDevice = if (mappedDevice == AudioDeviceType.EARPIECE && audioDevices.find { it.audioDeviceType == AudioDeviceType.WIRED_HEADSET } != null) AudioDevice(AudioDeviceType.WIRED_HEADSET) else AudioDevice(mappedDevice)
  ...
  private fun setAudioDevice(device: AudioDevice) {
    ...
    when (device.audioDeviceType) {
      AudioDeviceType.SPEAKER_PHONE -> setSpeakerphoneOn(true)
      AudioDeviceType.EARPIECE -> setSpeakerphoneOn(false)
      AudioDeviceType.WIRED_HEADSET -> setSpeakerphoneOn(false)
      AudioDeviceType.BLUETOOTH -> setSpeakerphoneOn(false)
